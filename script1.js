...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const setData = (value, finalData) => {
        ...
        if (fullName.length > 1) {
            ...
            //const Alias = fullName[fullName.length - 1] >= 3 ? (fullName[0].slice(0, 1) + fullName[fullName.length - 1].slice(0, 3)) : (fullName[0].slice(0, (5 - fullName[fullName.length - 1])) + fullName[fullName.length - 1].slice(0, 3));
            let Alias = lastName.length >= 3 ? (firstName.slice(0, 1) + lastName.slice(0, 3)) : firstName.slice(0, (5 - lastName.length)) + lastName.slice(0, lastName.length)
        ...
        //Division
        let division = value['Business '] ? value['Business '] : value['Business Area '];

        division = division ? division.toUpperCase() : ''
        console.log(division)
        finalData['Division_DIV__c'] = getDivisions(division) ? getDivisions(division) : ''
        finalData['Business_Area_UAM__c'] = finalData['Division_DIV__c']

        //Business_Unit_BU__c
        let business_area = value['Division'];
        business_area = business_area ? business_area.toUpperCase() : ''
        finalData['Business_Unit_BU__c'] = getBusinessUnits(business_area) ? getBusinessUnits(business_area) : ''

        //Product Groups
        let productGroup = value['Product Group'];
        //productGroup.toUpperCase();
        finalData['Product_Group_PG__c'] = getProductGroups(value['Product Group']) ? getProductGroups(value['Product Group']) : ''

        //Department_type

        finalData['Department_Type__c'] = getDepartmentType(value['Department type']) ? getDepartmentType(value['Department type']) : ''
        ...
        //finalData['Manager:User:Username'] = value['Line Manager']
        finalData['Sales_Territory_Name__c'] = value['Sales Territory Name']
        let role = value['Role']
        role = role ? role.trim() : '';
        console.log(role)
        if (doesRoleExist(role)) {
            console.log(role)
            finalData[`UserRole:UserRole:Name`] = role
        }
        let profile = value['Profile']
        profile = profile ? profile.trim() : '';
        if (doesProfileExist(profile)) {
            console.log("exists")
            finalData[`Profile:Profile:Name`] = profile
        }
